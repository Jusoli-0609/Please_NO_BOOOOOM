...
std::vector<Quiz> KimDongHyunManagerQuiz = {
    {
        "int arr[3] = {10, 20, 30}; int* p = arr; 일 때, *(p + 1) + arr[2]의 값은?",
        {"30", "40", "50", "60"},
        3
    },
    {
        "void func(int& a) { a += 10; } 실행 후 int x = 5; func(x); 호출 시 x의 값은?",
        {"5", "10", "15", "에러 발생"},
        3
    },
    {
        "std::vector<int> v = {1, 2, 3}; v.pop_back(); v.push_back(4); 실행 후 v.size()는?",
        {"2", "3", "4", "5"},
        2
    },
    {
        "int a = 5, b = 10; bool res = (a > 3 && b++ < 10); 실행 후 b의 값은?",
        {"9", "10", "11", "12"},
        3
    }
};
...
std::vector<Quiz> MoonSeungHoManagerQuiz = {
    {
        "int a = 10; int* p = &a; int** pp = &p; 일 때 **pp + 5의 값은?",
        {"15", "a의 주소값", "p의 주소값", "컴파일 에러"},
        1
    },
    {
        "부모 클래스 포인터로 자식 객체 해제 시 메모리 누수를 막기 위한 키워드는?",
        {"static", "virtual", "override", "const"},
        2
    },
    {
        "int* ptr = new int[5]; 동적 할당 메모리의 올바른 해제 구문은?",
        {"delete ptr;", "free(ptr);", "delete[] ptr;", "remove(ptr);"},
        3
    },
    {
        "std::map<string, int>에서 존재하지 않는 키에 map[key] 접근 시 현상은?",
        {"런타임 에러", "NULL 반환", "기본값으로 자동 생성", "컴파일 에러"},
        3
    },
    {
        "64비트 환경에서 double* p; 의 sizeof(p) 크기는 몇 바이트인가?",
        {"4바이트", "8바이트", "16바이트", "가변적"},
        2
    },
    {
        "객체 생성 없이 클래스에서 직접 호출 가능한 멤버 선언 키워드는?",
        {"inline", "explicit", "static", "friend"},
        3
    }
};