...
template<typename T>//1-2 복사 생성자
...
void Inventory<T>::Use_Item(Player& player, Monster& monster)
{
    ...
    T* selected_item = Get_Item_By_Index(Vector_Index);
    if (selected_item == nullptr)
    {
        cout << "잘못된 아이템 번호다!" << endl;
        return;
    }
    string selected_item_name = selected_item->_Item_Name;
...
void Inventory<T>::Use_Random_Item_In_Battle(Player& player, Monster& monster)
{
    ...
    T* selected_item = Get_Item_By_Index(Random_Item_use);
    if (selected_item == nullptr)
    {
        return;
    }
